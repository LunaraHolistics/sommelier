...
app.get("/api/cardapio", (req, res) => {
  try {
    const cardapioPath = path.join(__dirname, "data", "cardapio.json");
    const cardapio = JSON.parse(fs.readFileSync(cardapioPath, "utf-8"));
    const { categoria, search, tags } = req.query;
    let result = Array.isArray(cardapio) ? cardapio : [];
...
// Criar novo prato
app.post("/api/cardapio", authManager, (req, res) => {
  try {
    const cardapioPath = path.join(__dirname, "data", "cardapio.json");
    const cardapio = JSON.parse(fs.readFileSync(cardapioPath, "utf-8"));

    const novoPrato = {
      id: "PRATO" + String(cardapio.length + 1).padStart(3, "0"),
      ...req.body,
...
// Atualizar prato
app.put("/api/cardapio/:id", authManager, (req, res) => {
  try {
    const cardapioPath = path.join(__dirname, "data", "cardapio.json");
    const cardapio = JSON.parse(fs.readFileSync(cardapioPath, "utf-8"));
...
app.patch("/api/cardapio/:id/status", authManager, (req, res) => {
  try {
    const cardapioPath = path.join(__dirname, "data", "cardapio.json");
    const cardapio = JSON.parse(fs.readFileSync(cardapioPath, "utf-8"));
    const { status } = req.body;
    ...
    cardapio[index].status = status;
    cardapio[index].updatedAt = new Date().toISOString();
...
// Excluir prato
app.delete("/api/cardapio/:id", authManager, (req, res) => {
  try {
    const cardapioPath = path.join(__dirname, "data", "cardapio.json");
    const cardapio = JSON.parse(fs.readFileSync(cardapioPath, "utf-8"));
    const id = req.params.id;
